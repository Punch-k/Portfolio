// Global desktop check to prevent heavy hover scripts on mobile
...
// 2. CURSOR & AMBIENT AURA (MOBILE OPTIMIZED)
if (isDesktop) {
    ...
    (function loop() {
        ...
        if (aura) aura.style.transform = `translate(calc(${ax}px - 50%), calc(${ay}px - 50%))`;
...
(function () {
    ...
    function calcEOQ(D, S, H) { return Math.sqrt((2 * D * S) / H); }
    ...
    function calcROP(avgD, avgL, ss) { return (avgD / 365) * avgL + ss; }
    ...
    function calcStockoutRiskCost(D, Q, z, sigmaD, avgL, unitCost, servicePct) {
        ...
        return expectedShortage * cyclesPerYear * unitCost * 0.25; 
    }
        ...
        const cfg = {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: ssCosts,
                    backgroundColor: ssCosts.map((_, i) => i === currentIdx ? GOLD : 'rgba(201,168,76,0.18)'),
                    borderColor: ssCosts.map((_, i) => i === currentIdx ? GOLD : 'rgba(201,168,76,0.35)'),
                    borderWidth: 1,
                }]
            },
            options: {
    ...
    function _buildSensitivity(p, H, z, baseEOQ, baseSS, baseTotal, avgD) {
        ...
        scenarios.forEach(sc => {
            ...
            const diffStr = diff === 0 ? '--' : `<span class="${diff < 0 ? 'ssens-delta-pos' : 'ssens-delta-neg'}">${diff < 0 ? '(-) ' : '(+) '} ${Math.abs(diffPct)}%</span>`;
...
(function () {
    ...
    function readQInputs() {
        const g = id => parseFloat(document.getElementById(id).value) || 0;
        return {
            arrivalInterval: g('q-arrival'),
            serviceTime: g('q-service'),
            numServers: Math.max(1, Math.round(g('q-servers'))),
            simTime: g('q-simtime'),
            staffCost: g('q-staffcost'),
        };
    }
    ...
    function runMMC(arrivalInterval, serviceTime, numServers, simTime) {
        ...
        const arrivalRate = 1 / arrivalInterval;
        ...
        const waitTimes = [];
        const queueSnaps = [];
        const serverFree = new Array(numServers).fill(0);
        let totalBusyTime = 0;
    ...
    function drawQueueChart(snaps) {
        ...
        qCharts.queue = new Chart(document.getElementById('q-chart-queue'), {
            ...
            data: {
                labels,
                datasets: [
                    { label: 'Queue Length', data, borderColor: GOLD, borderWidth: 1.5, pointRadius: 0, fill: true, backgroundColor: 'rgba(201,168,76,0.06)', tension: 0.3 },
                    { label: 'Avg Queue', data: labels.map(() => parseFloat(avgQ.toFixed(2))), borderColor: ORG, borderWidth: 1, borderDash: [4, 4], pointRadius: 0, fill: false }
                ]
            },
            options: {
                responsive: true, maintainAspectRatio: false,
                ...
                plugins: {
                    ...
                    tooltip: { backgroundColor: 'rgba(8,9,14,.95)', borderColor: 'rgba(201,168,76,.3)', borderWidth: 1, titleColor: 'rgba(237,233,224,.55)', bodyColor: GOLD, padding: 9, titleFont: { size: 10, family: 'DM Sans' }, bodyFont: { size: 11, family: 'DM Sans' } }
                },
                scales: {
    ...
    function drawSensitivityChart(sweep, currentServers) {
        ...
        const colors = sweep.map(s => s.servers === currentServers ? GOLD : 'rgba(201,168,76,0.25)');
        const borderColors = sweep.map(s => s.servers === currentServers ? GOLD : 'rgba(201,168,76,0.45)');
        ...
        qCharts.sensitivity = new Chart(document.getElementById('q-chart-sensitivity'), {
            ...
            data: {
                labels,
                datasets: [{ label: 'Avg Wait (min)', data: waits, backgroundColor: colors, borderColor: borderColors, borderWidth: 1 }]
            },
            options: {
                responsive: true, maintainAspectRatio: false,
                animation: { duration: 600 },
                plugins: {
                    legend: { display: false },
                    tooltip: {
                        backgroundColor: 'rgba(8,9,14,.95)', borderColor: 'rgba(201,168,76,.3)', borderWidth: 1, titleColor: 'rgba(237,233,224,.55)', bodyColor: GOLD, padding: 9,
                        callbacks: { title: ctx => ctx[0].label, label: ctx => ' Avg Wait: ' + ctx.parsed.y.toFixed(2) + ' min' }
                    }
                },
                scales: {
                    x: { ticks: { color: FG, font: { size: 9, family: 'DM Sans' } }, grid: { color: FG_DIM }, border: { color: FG_DIM } },
                    y: { ticks: { color: FG, font: { size: 9, family: 'DM Sans' } }, grid: { color: FG_DIM }, border: { color: FG_DIM }, min: 0, title: { display: true, text: 'Avg Wait (min)', color: FG, font: { size: 9 } } }
                }
            }
        });
    }
...
(function () {
    ...
    function drawFCFChart(sched) {
        ...
        dcfCharts.fcf = new Chart(document.getElementById('d-chart-fcf'), {
            data: {
                labels,
                datasets: [
                    { type: 'bar', label: 'FCF ($K)', data: fcfs, backgroundColor: 'rgba(201,168,76,0.25)', borderColor: Au, borderWidth: 1.5, yAxisID: 'y' },
                    { type: 'line', label: 'Revenue ($K)', data: revs, borderColor: 'rgba(237,233,224,0.5)', borderWidth: 1.5, pointRadius: 3, fill: false, tension: .3, yAxisID: 'y' },
                    { type: 'line', label: 'EBITDA ($K)', data: ebs, borderColor: Bu, borderWidth: 1.5, pointRadius: 3, fill: false, tension: .3, yAxisID: 'y' },
                ]
            },
            options: {
                responsive: true, maintainAspectRatio: false,
                ...
                plugins: {
                    ...
                    tooltip: {
                        backgroundColor: 'rgba(8,9,14,.95)', borderColor: 'rgba(201,168,76,.3)', borderWidth: 1, titleColor: 'rgba(237,233,224,.55)', bodyColor: Au, padding: 9,
    ...
    function drawFootballField(p) {
        ...
        const datasets = tgrs.map((tg, ti) => {
            ...
            return {
                label: tgLabel, data, backgroundColor: color, borderColor: color, borderWidth: 1
            };
        });
        ...
        dcfCharts.football = new Chart(document.getElementById('d-chart-football'), {
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        backgroundColor: 'rgba(8,9,14,.95)', borderColor: 'rgba(201,168,76,.3)', borderWidth: 1, titleColor: 'rgba(237,233,224,.55)', bodyColor: Au, padding: 9,
                        callbacks: { title: ctx => 'WACC: ' + ctx[0].label, label: ctx => ' ' + ctx.dataset.label + ' EV: ' + fmtK(ctx.parsed.y) }
                    }
                },
                scales: {
                    x: { ticks: { color: FC, font: { size: 9, family: 'DM Sans' } }, grid: { color: FD }, border: { color: FD }, title: { display: true, text: 'WACC', color: FC, font: { size: 9 } } },
                    y: { ticks: { color: FC, font: { size: 9, family: 'DM Sans' }, callback: v => fmtK(v) }, grid: { color: FD }, border: { color: FD }, title: { display: true, text: 'Enterprise Value ($K)', color: FC, font: { size: 9 } } }
                }
            }
        });
    }
...
// ═══════════════════════════════════════════════════════════════
var _lastInvResults = null;   
var _lastQueueResults = null; 

function _cacheInvResults(p, eoq, ss, rop, grandTotal, avgD) {
    _lastInvResults = {
        eoq: eoq, ss: ss, rop: rop, totalCost: grandTotal, unitCost: p.C,
        leadTime: p.L, demand: p.D, svcPct: p.svcPct, avgD: avgD
    };
}

function _cacheQueueResults(p, r) {
    _lastQueueResults = {
        served: r.served, avgWait: r.avgWait, util: r.util, staffCost: p.staffCost,
        numServers: p.numServers, simTime: p.simTime, arrivalInterval: p.arrivalInterval,
...
window.carryInvToQueue = function () {
    ...
    var demandPerMin = demandPerDay / 480; 
    ...
    var simTime = Math.max(60, Math.min(480, cycledays * 8 * 60 / 60)); 
...
window.carryQueueToDCF = function () {
    ...
    var annualCustomers = q.served * (525600 / q.simTime); 
    var revenueK = Math.round(annualCustomers * unitCost / 1000);
    revenueK = Math.max(100, Math.min(500000, revenueK));

    var util = q.util;
    var growth1 = util > 85 ? 8 : util > 70 ? 14 : 20;  
    var growth2 = Math.round(growth1 * 0.55);              
...
(function () {
    ...
    slider.addEventListener('touchstart', e => {
        ...
        tScrollLeft = slider.scrollLeft;
    }, { passive: true });

    slider.addEventListener('touchmove', e => {
        const dx = tStartX - e.touches[0].pageX;
        slider.scrollLeft = tScrollLeft + (dx * 1.2); 
    }, { passive: true });
})();

// 24. FLOATING CONNECT PILL
...
(function initSupplyGame() {
    ...
    function jump(e) {
        ...
        if(e && (e.type === 'keydown' || e.type === 'touchstart')) e.preventDefault();
   ...
   function drawRoad() {   
    ...
    function loop(ts) {
        ...
        if (state === 'running') {
            ...
            bumpTimer++; 
            const bumpInterval = Math.max(45, 130 - speed * 6); 
