...
// 3 + 4 + 8 + 13 + float-CTA — CONSOLIDATED SCROLL HANDLER (was 6 separate listeners)
...
function onScroll(){
  ...
  const pctRead=Math.min(sy/docH*100,100);

  // Progress bar
  sprog.style.width=pctRead+'%';
  ...
  if(tlLine&&expSection){
    ...
    tlLine.style.height=(Math.min(1,scrolled/expSection.offsetHeight)*100)+'%';
...
// scrub scroll is handled inside consolidated onScroll below
let scrubTimer=null;
...
// 8. NAV DOT CLICKS & SMOOTH ANCHOR SCROLLING
...
// FAQ ACCORDION — handled by existing block below (section 25)
...
// tlLine & expSection timeline scroll handled in consolidated onScroll
...
// 24. FLOATING CONNECT PILL + read-time — handled in consolidated onScroll
